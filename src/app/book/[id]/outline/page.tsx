    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white mt-4">Loading outline...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <h1 className="text-4xl font-bold text-white mt-2">📋 Outline Editor</h1>
            <h2 className="text-xl text-purple-200 mt-2">{book.title}</h2>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-2xl font-bold text-white mb-4">🤖 AI Outline Generation</h3>
            <div className="space-y-4">
              <textarea
                ...
                className="w-full h-32 p-4 bg-gray-700 text-white rounded border border-gray-600 focus:border-purple-500 focus:outline-none resize-none"
              />
              <button
                onClick={generateAIOutline}
                disabled={isGenerating || !aiContext}
                className="w-full px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 text-white rounded-lg font-semibold transition-colors"
              >
                {isGenerating ? '🤖 Generating...' : '✨ Generate AI Outline'}
              </button>
              {!aiContext && (
                <p className="text-yellow-400 text-sm">AI Context not available. Check your setup.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-2xl font-bold text-white mb-4">➕ Add Manual Item</h3>
            <div className="space-y-4">
              <input
                type="text"
                value={newItemTitle}
              ...
              <textarea
                ...
                className="w-full h-24 p-3 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 focus:outline-none resize-none"
              />
              <button
                onClick={addManualItem}
                disabled={!newItemTitle.trim()}
                className="w-full px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white rounded font-semibold transition-colors"
              >
                Add to Outline
              </button>
            </div>
        ...
        <div className="mt-8 bg-gray-800 rounded-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-white">📚 Current Outline</h3>
            {outline.length > 0 && (
              <button
          ...
          {outline.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-400 text-lg mb-4">No outline items yet</p>
